  ...
  return (
    ...
    >
      <Link
        ...
        className="group flex flex-col bg-white rounded-xl border border-stone-200 overflow-hidden hover:border-brand-300 hover:shadow-xl transition-shadow"
      >
        ...
        <div className="flex flex-col flex-1 p-5">
          <div className="flex items-start justify-between gap-2 mb-2">
            <h3 className="font-display text-xl font-semibold text-stone-900 leading-tight group-hover:text-brand-600 transition-colors tracking-tight">
              {nombreMostrar}
            </h3>
          </div>
          ...
          <div className="flex flex-col gap-2 text-sm text-stone-600 mt-auto">
            <div className="flex items-center gap-1.5">
              <MapPin className="h-4 w-4 flex-shrink-0 text-stone-500" aria-hidden="true" />
              <span className="truncate">{ubicacion.barrio}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Calendar className="h-4 w-4 flex-shrink-0 text-stone-500" aria-hidden="true" />
              <span>{formatearFechaRelativa(fechaPublicacion)}</span>
            </div>
          </div>

          {recompensa && recompensa > 0 && (
            <div className="mt-4 inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-amber-50 text-amber-800 border border-amber-200 self-start">
